...
static void test_storage_backend_atomic_slots(void) {
  uint8_t blob_a[8] = {1u, 2u, 3u, 4u, 5u, 6u, 7u, 8u};
  uint8_t blob_b[8] = {9u, 9u, 9u, 9u, 9u, 9u, 9u, 9u};
  uint8_t out[8] = {0};
  size_t out_len = sizeof(out);
  uint32_t version = 0u;
  storage_backend_debug_t dbg = {0};

  storage_backend_init();
  assert(storage_backend_write_atomic(blob_a, sizeof(blob_a), 1u));
  assert(storage_backend_read_latest(out, sizeof(out), &out_len, &version));
  assert(out_len == sizeof(blob_a));
  assert(version == 1u);
  assert(memcmp(out, blob_a, sizeof(blob_a)) == 0);

  assert(storage_backend_write_atomic(blob_b, sizeof(blob_b), 2u));
  out_len = sizeof(out);
  assert(storage_backend_read_latest(out, sizeof(out), &out_len, &version));
  assert(version == 2u);
  assert(memcmp(out, blob_b, sizeof(blob_b)) == 0);

  assert(storage_backend_debug_state(&dbg));
