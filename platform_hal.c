...
void platform_hal_init(void) {
    ...
    // Call the weak HAL_MspInit function to initialize the MSP for peripherals
    HAL_MspInit();
}
...
void platform_hal_led_set(platform_hal_led_t led, bool on) {
    switch (led) {
        ...
        case PLATFORM_HAL_LED_ACTIVITY:
            ...
            break;
        case PLATFORM_HAL_LED_UNLOCKED: // Assuming unlocked LED is also on PA1
            HAL_GPIO_WritePin(GPIOA, GPIO_PIN_1, on ? GPIO_PIN_SET : GPIO_PIN_RESET);
            break;
        case PLATFORM_HAL_LED_ERROR: // Assuming error LED is on PA0
            HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0, on ? GPIO_PIN_SET : GPIO_PIN_RESET);
            break;
        case PLATFORM_HAL_LED_OFF: // Turn off all LEDs
...
// Simulated touch input for testing purposes
static bool simulated_touch_pressed = false;
static bool simulated_touch_held = false;

void platform_hal_touch_set_simulated(bool pressed, bool held) {
    simulated_touch_pressed = pressed;
    simulated_touch_held = held;
}

bool platform_hal_touch_held(void) {
    if (simulated_touch_pressed) {
        return simulated_touch_held;
    }
...
// These are intentionally left empty here as they should be implemented in the application code.
// The HAL_Init() function calls HAL_MspInit() which will then call these weak functions.
void HAL_I2C_MspInit(I2C_HandleTypeDef *hi2c) { (void)hi2c; /* Suppress unused parameter warning */ }
void HAL_I2C_MspDeInit(I2C_HandleTypeDef *hi2c) { (void)hi2c; /* Suppress unused parameter warning */ }
void HAL_SPI_MspInit(SPI_HandleTypeDef *hspi) { (void)hspi; /* Suppress unused parameter warning */ }
void HAL_SPI_MspDeInit(SPI_HandleTypeDef *hspi) { (void)hspi; /* Suppress unused parameter warning */ }
void HAL_OSPI_MspInit(OSPI_HandleTypeDef *hospi) { (void)hospi; /* Suppress unused parameter warning */ }
void HAL_OSPI_MspDeInit(OSPI_HandleTypeDef *hospi) { (void)hospi; /* Suppress unused parameter warning */ }
void HAL_PCD_MspInit(PCD_HandleTypeDef *hpcd) { (void)hpcd; /* Suppress unused parameter warning */ }
void HAL_PCD_MspDeInit(PCD_HandleTypeDef *hpcd) { (void)hpcd; /* Suppress unused parameter warning */ }
